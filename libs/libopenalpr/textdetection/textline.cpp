...
namespace alpr
{
  ...
  TextLine::TextLine(std::vector<cv::Point2f> textArea, std::vector<cv::Point2f> linePolygon, cv::Size imgSize) {
    ...
    for (unsigned int i = 0; i < textArea.size(); i++)
      textAreaInts.push_back(Point((int)round(textArea[i].x), (int)round(textArea[i].y)));
    for (unsigned int i = 0; i < linePolygon.size(); i++)
      linePolygonInts.push_back(Point((int)round(linePolygon[i].x), (int)round(linePolygon[i].y)));
  ...
  void TextLine::initialize(std::vector<cv::Point> textArea, std::vector<cv::Point> linePolygon, cv::Size imgSize) {
    if (textArea.size() > 0)
    {
      ...
      {
        linePolygon[0].x = 0;
        linePolygon[0].y = (int)topLine.getPointAt((float)linePolygon[0].x);
      }
      if (linePolygon[1].x != imgSize.width)
      {
        linePolygon[1].x = imgSize.width;
        linePolygon[1].y = (int)topLine.getPointAt((float)linePolygon[1].x);
      }
      if (linePolygon[2].x != imgSize.width)
      {
        linePolygon[2].x = imgSize.width;
        linePolygon[2].y = (int)bottomLine.getPointAt((float)linePolygon[2].x);
      }
      if (linePolygon[3].x != 0)
      {
        linePolygon[3].x = 0;
        linePolygon[3].y = (int)bottomLine.getPointAt((float)linePolygon[3].x);
      }
      ...
      Point midpoint = Point((int)x, (int)bottomLine.getPointAt((float)x));
      Point acrossFromMidpoint = topLine.closestPointOnSegmentTo(midpoint);
      this->lineHeight = (float)distanceBetweenPoints(midpoint, acrossFromMidpoint);
  ...
  cv::Mat TextLine::drawDebugImage(cv::Mat baseImage) {
    ...
    fillConvexPoly(debugImage, linePolygon.data(), (int)linePolygon.size(), Scalar(0,0,165));

    fillConvexPoly(debugImage, textArea.data(), (int)textArea.size(), Scalar(125,255,0));
