...
namespace alpr
{
  ...
  {
    const float MIN_AREA_TO_IGNORE = 0.65f;
  ...
  std::vector<cv::Point> LineFinder::calculateCroppedRegionForHistogram(cv::Size imageSize, std::vector<cv::Point> charArea) {
      ...
      if (topLine.angle < 0)
      {
        float distance_from_top = (float)topLine.p1.y;
        new_top = topLine.getParallelLine(distance_from_top);
        
        float distance_from_bottom = (float)(imageSize.height - topLine.p2.y);
        new_bottom = topLine.getParallelLine(-1 * distance_from_bottom);
      }
      else
      {
        float distance_from_top = (float) topLine.p2.y;
        new_top = topLine.getParallelLine(distance_from_top);
        
        float distance_from_bottom = (float)(imageSize.height - topLine.p1.y);
        new_bottom = topLine.getParallelLine(-1 * distance_from_bottom);
      }
  ...
  std::vector<cv::Point> LineFinder::findNextBestLine(cv::Size imageSize, std::vector<cv::Point> bestLine) {
      ...
      Size cropped_quad_size((int)distanceBetweenPoints(histogramArea[0], histogramArea[1]), (int)distanceBetweenPoints(histogramArea[0], histogramArea[3]));
      ...
      outputQuad.push_back(Point2f((float)0, (float)0));
      outputQuad.push_back(Point2f((float)cropped_quad_size.width, (float)0));
      outputQuad.push_back(Point2f((float)cropped_quad_size.width, (float)cropped_quad_size.height));
      outputQuad.push_back(Point2f((float)0,(float)cropped_quad_size.height));
      ...
      int transformed_best_line_start = (int)round(transformed_best_line[0].y);
      int transformed_best_line_end =(int) round(transformed_best_line[3].y);
      int transformed_best_line_width = transformed_best_line_end - transformed_best_line_start;
      int transformed_best_line_variance = (int) (((float) transformed_best_line_width) * 0.25f);
      
      

      float lowest_width_diff = 99999999999.0f;
      ...
      for (unsigned int i = 0; i < pipeline_data->thresholds.size(); i++)
      {
        ...
        float max_deviation_percent = 0.30f;
        ...
        for (unsigned int hitidx = 0; hitidx < histogram_hits.size(); hitidx++)
        {
          ...
          float hit_width = (float)(histogram_hits[hitidx].second -  histogram_hits[hitidx].first);
      ...
      LineSegment secondline_top = orig_top_line.getParallelLine((float)(best_secondline_top_pixel_offset_from_bestline_top + 1));
      LineSegment secondline_bottom = orig_top_line.getParallelLine((float)(best_secondline_bottom_pixel_offset_from_bestline_top - 1));
  ...
  vector<Point> LineFinder::getBestLine(const TextContours contours, vector<CharPointInfo> charPoints)
  {
    ...
    float medianCharHeight = (float)median(charheights.data(), (int)charheights.size());
    ...
    {
      float SCORING_MIN_THRESHOLD = 0.97f;
      float SCORING_MAX_THRESHOLD = 1.03f;

      int curScore = 0;
      for (unsigned int charidx = 0; charidx < charPoints.size(); charidx++)
      {
        float topYPos = topLines[i].getPointAt((float)charPoints[charidx].top.x);
        float botYPos = bottomLines[i].getPointAt((float)charPoints[charidx].bottom.x);
          ...
          (curScore == bestScore && topLines[i].length > bestScoreDistance))
      {
        ...
        bestScoreDistance = (int)topLines[i].length;
      }
    }
  ...
  std::vector<cv::Point> LineFinder::extendToEdges(cv::Size imageSize, std::vector<cv::Point> charArea) {
    ...
    Point topLeft 		= Point(0, (int)top.getPointAt((float)0) );
    Point topRight 		= Point(imageSize.width, (int)top.getPointAt((float)imageSize.width));
    Point bottomRight 	= Point(imageSize.width, (int)bottom.getPointAt((float)imageSize.width));
    Point bottomLeft 	= Point(0, (int)bottom.getPointAt((float)0));
