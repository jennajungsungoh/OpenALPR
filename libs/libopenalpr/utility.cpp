...
namespace alpr
{
  ...
  Rect expandRect(Rect original, int expandXPixels, int expandYPixels, int maxX, int maxY)
  {
    ...
    float halfX = (float)(round((float) expandXPixels / 2.0));
    float halfY = (float)(round((float) expandYPixels / 2.0));
    expandedRegion.x = (int)(expandedRegion.x - halfX);
    expandedRegion.width =  expandedRegion.width + expandXPixels;
    expandedRegion.y = (int)(expandedRegion.y - halfY);
  ...
  {
    unsigned int numRows = (unsigned int)(ceil((float) images.size() / (float) numColumns));
    ...
    {
      if (i < images.size())
        images[i].copyTo(dashboard(Rect((i%numColumns) * images[i].cols, (int)(floor((float) i/numColumns) * images[i].rows), images[i].cols, images[i].rows)));
      else
      {
        Mat black = Mat::zeros(images[0].size(), imageType);
        black.copyTo(dashboard(Rect((i%numColumns) * images[0].cols, (int)(floor((float) i/numColumns) * images[0].rows), images[0].cols, images[0].rows)));
      }
    }
  ...
  float angleBetweenPoints(Point p1, Point p2)
  {
    ...
    return (float)(atan2((float) deltaY, (float) deltaX) * (180 / CV_PI));
  }
  ...
  Size getSizeMaintainingAspect(Mat inputImg, int maxWidth, int maxHeight)
  {
    ...
    {
      return Size((int)(maxHeight * aspect), maxHeight);
    }
    else
    {
      return Size(maxWidth, (int)(maxWidth / aspect));
    }
  }
...
int levenshteinDistance (const std::string &s1, const std::string &s2, int max)
{
  ...
  void LineSegment::init(int x1, int y1, int x2, int y2)
  {
    ...
    if (p2.x - p1.x == 0)
      this->slope = 0.00000000001f;
    else
      this->slope = (float) (p2.y - p1.y) / (float) (p2.x - p1.x);

    this->length = (float) distanceBetweenPoints(p1, p2);
  ...
  Point LineSegment::closestPointOnSegmentTo(Point p)
  {
    ...
    float bottom = (float)distanceBetweenPoints(p2, p1);
  ...
  Point LineSegment::intersection(LineSegment line)
  {
    ...
    {
      // Line1 is vertical
      return Point(p1.x, (int)line.getPointAt((float)p1.x));
    }
    else if (line.p1.x == line.p2.x)
    {
      // Line2 is vertical
      return Point(line.p1.x, (int)getPointAt((float)line.p1.x));
    }
    else
    {
  ...
  cv::Point findClosestPoint(cv::Point2f* polygon_points, int num_points, cv::Point position)
  {
    ...
    for (int i = 0; i < num_points; i++)
    {
      ...
      unsigned int distance = (unsigned int)distanceBetweenPoints(pos, position);
