        ...
        for (int i = 0; i < results.plates.size(); i++)
        {
            ...
            if (TcpConnectedPort)
            {
                ...
                if (!found)
                {
                    ...
                    if ((result = (int)WriteDataTcp(ssl,TcpConnectedPort, (unsigned char*)&SendMsgHdr, sizeof(SendMsgHdr))) != sizeof(SendPlateStringLength))
                        printf("WriteDataTcp %d\n", result);
                    if ((result = (int)WriteDataTcp(ssl,TcpConnectedPort, (unsigned char*)results.plates[i].bestPlate.characters.c_str(), SendPlateStringLength)) != SendPlateStringLength)
                        printf("WriteDataTcp %d\n", result);
...
static void GetResponses(void)
{
 ...
 while ((BytesOnSocket = BytesAvailableTcp(TcpConnectedPort)) > 0)
 {
     ...
     BytesRead = ReadDataTcp(ssl,TcpConnectedPort, (unsigned char*)& ResponseBuffer[BytesInResponseBuffer], BytesOnSocket);
     if (BytesRead <= 0)
     {
