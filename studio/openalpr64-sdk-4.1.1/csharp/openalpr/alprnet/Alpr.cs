...
namespace AlprNet
{
    public class Alpr : IDisposable
    {
        ...
        if (hardware_acceleration == 0)
            ...
            catch (System.DllNotFoundException)
            {
                if (Environment.OSVersion.Platform == PlatformID.Unix)
                    Console.WriteLine("Could not find/load native library (libopenalpr.so)");
                else
        ...
        public string VehicleSignature(string filepath)
        {
            return VehicleSignature(System.IO.File.ReadAllBytes(filepath));
        }

        /// <summary>
        /// Generates a vehicle signature for a given image (e.g., JPG, PNG, etc) provided as an array of bytes
        /// </summary>
        /// <param name="image_bytes">The raw bytes that compose the image</param>
        /// <returns>A string value representing a signature of the vehicle</returns>
        /// <exception cref="InvalidOperationException">Thrown when the library has not been initialized</exception>
        public string VehicleSignature(byte[] image_bytes)
        {
            check_initialization();
            ...
            IntPtr resp_ptr = openalpr_vehiclesignature_encodedimage(this.native_instance, unmanagedArray, image_bytes.Length, roi, 1);
        ...
        public string VehicleSignature(System.Drawing.Image image)
        {
            check_initialization();
            ...
            IntPtr resp_ptr = openalpr_vehiclesignature_rawimage(this.native_instance, unmanagedArray, bytes_per_pixel, image.Width, image.Height, roi, 1);
        ...
        /// <summary>
        /// Compares two vehicle signatures to return a similarity score (between 0 and 1.0)
        /// </summary>
        /// <param name="image_bytes">The raw bytes that compose the image</param>
        /// <returns>A string value representing a signature of the vehicle</returns>
        /// <exception cref="InvalidOperationException">Thrown when the library has not been initialized</exception>
        public double VehicleSimilarity(string signatureA, string signatureB)
        {
            return openalpr_vehiclesignature_similarity(this.native_instance, signatureA, signatureB);
        }
        ...
        [DllImport("libopenalpr")]
        private static extern IntPtr openalpr_vehiclesignature_rawimage(IntPtr instance, IntPtr pixelData, int bytesPerPixel, int imgWidth, int imgHeight, NativeROI roi, int high_resolution);

        [DllImport("libopenalpr")]
        private static extern IntPtr openalpr_vehiclesignature_encodedimage(IntPtr instance, IntPtr bytes, long length, NativeROI roi, int high_resolution);

        [DllImport("libopenalpr")]
        private static extern double openalpr_vehiclesignature_similarity(IntPtr instance, [MarshalAs(UnmanagedType.LPStr)] string signatureA, [MarshalAs(UnmanagedType.LPStr)] string signatureB);
    }
}
